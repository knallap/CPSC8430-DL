...
def s_split(sentence, word_dict, w2i):
    ...
    sentence = [w2i.get(word, 3) for word in sentence]  # Replace unknown words with <UNK> token
    sentence.insert(0, 1)  # <SOS>
...
class DecoderRNN(nn.Module):
    ...
    def forward(self, encoder_last_hidden_state, encoder_output, targets=None, mode='train', tr_steps=None):
        ...
        for i in range(seq_len - 1):
            threshold = self.teacher_forcing_ratio(tr_steps)
...
def train(model, epoch, loss_fn, parameters, optimizer, train_loader):
    ...
    for batch_idx, batch in enumerate(train_loader):
        ...
        ground_truths = ground_truths[:, 1:]  # Shift target sentences to ignore <SOS>
...
def main():
    ...
    train_dataloader = DataLoader(dataset=train_dataset, batch_size=256, shuffle=True, num_workers=16, collate_fn=minibatch)
    ...
    torch.save(model, 'knallap_model.h5')
